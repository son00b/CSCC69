...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if (frame == -1) { // Didn't find a free page.
		...
		// retrieve to-be-evicted page
		pgtbl_entry_t* bye = coremap[frame].pte;

		// clean eviction,
		if (! (bye->frame & PG_DIRTY)) {
			evict_clean_count ++;

		// dirty eviction, write victim PTE to swap
		} else {
			evict_dirty_count ++;
			int offset = swap_pageout(frame, bye->swap_off);

			// if swap succeeded, write victim on swap, and update page table
			if (offset != INVALID_SWAP) {
				bye->frame |= PG_ONSWAP;
				bye->swap_off = offset;
			}
		}
		bye->frame &= ~PG_VALID; // make to-be-evicted page invalid
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// initilize next level if page directory entry is empty
	if (pgdir[idx].pde == 0) {
		pgdir[idx] = init_second_level();
	}

	// Use top-level page directory to get pointer to 2nd-level page table
	pgtbl_entry_t *pter = (pgtbl_entry_t *)pgdir[idx].pde;
	...
	if (! (p->frame & PG_VALID)) {
		...
		// if p is invalid and on swap, get data from swap and put it in frame
		if (p->frame & PG_ONSWAP) {
			swap_pagein(frame, p->swap_off);

		// if p is invalid and not on swap, initialize frame
		} else {
	...
	p->frame |= PG_VALID; // set valid
	p->frame |= PG_REF; // set ref
