...
// node in double linked list (for LRU)
typedef struct node {
	struct node *pre;
	struct node *nxt;
	int index; // index the page in the coremap
	pgtbl_entry_t entry; // page table entry 
} Node;

// item (double linked list node) in the tracefile (for Opt)
typedef struct traceItem {
	struct traceItem *nxt;
	struct traceItem *pre;
	int index; // index of the page referenced in the coremap
	int time; // time of the page referenced
	addr_t addr; // address of the page referenced
} TraceItem;
...
struct frame {
	...
	int index; // For FIFO & CLOCK: the 'pointer'/'clockhand'

	Node* head; // For LRU: the link list of frames, in order from newest to oldest

	TraceItem *cur; // For Opt: the current item in tracefile
	int lines; // For Opt: number of lines in tracefile
	addr_t *addrs; // For Opt: Array of addresses such that addrs[i] is the address of the frame at time i
	int *memory; // For Opt: Array of timeslices such that the memory[i] is the next time the frame
