...
MODULE_AUTHOR("Aditi Damle, Heng Liu, Ya-Tzu Wang");
MODULE_LICENSE("GPL");
...
{
	// Get the current pid, add synchronization and  delete the pid.
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// Fetch the current system call from the ax register.
	long unsigned int currSyscall = reg.ax;
	
	// The message needs to be logged only when monitored = 1 or monitored = 2 conditions are met.
	if((check_pid_monitored(currSyscall, current->pid) == 1) || (table[currSyscall].monitored == 2)){
		log_message(current->pid, currSyscall, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}
	table[currSyscall].intercepted = 1;

	// Return the original call.
	return table[currSyscall].f(reg);
}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	/*************************************************************
	THIS SECTION IS FOR THE BASIC ERROR CHECKS FOR THE PARAMETERS 
						AND PERMISSIONS.
	**************************************************************/
	...
	// Checks for the syscall parameter.
	if ((syscall == MY_CUSTOM_SYSCALL) || (syscall < 0) || (syscall > NR_syscalls)) {
		return -EINVAL;
	}

	// Permission check for first 2 commands.
	if((cmd == REQUEST_SYSCALL_INTERCEPT) || (cmd == REQUEST_SYSCALL_RELEASE)){
		// not root
		if (current_uid() != 0){
			return -EPERM;
	...
	/*************************************************************
	THIS SECTION IS FOR EXECUTING COMMANDS
	**************************************************************/
	switch (cmd){

		case REQUEST_SYSCALL_INTERCEPT :
			// Return EBUSY if the call is already intercepted.
			if (table[syscall].intercepted == 1){
				return -EBUSY;
			}

			// Save the original syscall and then replace the syscall in the 
			// system calls table with our interpretor.
		...
		case REQUEST_SYSCALL_RELEASE :
			// check if the call was even intercepted in the first place.
			if (table[syscall].intercepted == 0){
				return -EINVAL;
			}

			// Replace the original call in the systems call table
			spin_lock(&calltable_lock);
		...
		case REQUEST_START_MONITORING :
            ...
            if (pid != 0) {
                ...
                // Return EBUSY if the pid is already monitored.
                ...
                // If pid is not monitored, then monitor by adding the pid to syscall's list of monitored pids.
        ...
        case REQUEST_STOP_MONITORING :
			...
			if (table[syscall].intercepted == 0){
				...
				if (pid != 0) {
					...
					// If not monitored then return EINVAL.
					// Otherwise change the monitored flag and remove the pid.
					...
					// Change the monitored flag to appropriate vaule.

					if (table[syscall].monitored == 2) {
					// set to 1 if this is not the only pid
						table[syscall].monitored = 1;
						// otherwise set to 0
					} else {
					...
					// Delete the pid from syscall's list of monitored pids
					spin_lock(&pidlist_lock);
					...
					// pid = 0 and all pids should be stopped monitoring.
					spin_lock(&calltable_lock);
					if (table[syscall].monitored == 2) {
...
static int init_function(void) {
	// get the original and the exit system calls
	//spin_lock_init(&myspinlock);
	int index = 1;

	// set the lock for the table
	spin_lock(&calltable_lock);
	...
	spin_lock(&pidlist_lock);
	// fill in the table.	
	while(index < NR_syscalls + 1){
		//table[index].f = sys_call_table[i];
	...
	// free the lock for the pidlist & table so other threads/processes can use it as well.
	spin_unlock(&pidlist_lock);
	spin_unlock(&calltable_lock);
...
{    
	...
	while (index < NR_syscalls) {
        if (table[index].intercepted == 1) {            
            my_syscall(REQUEST_SYSCALL_RELEASE, index, index);
        }
